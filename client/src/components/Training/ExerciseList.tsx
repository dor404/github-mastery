...
import { basicBranchingExample, mergingExample, advancedWorkflowExample } from '../../data/branchVisualizationData';
...
const ExerciseList: React.FC = () => {
  ...
  // Function to get visualization data based on exercise difficulty
  const getVisualizationData = (exercise: Exercise) => {
    // First check if exercise has its own visualization data
    if (exercise.visualization) {
      return exercise.visualization;
    }
    
    // Otherwise choose based on difficulty
    switch (exercise.difficulty) {
      case 'beginner':
      case 'elementary':
        return basicBranchingExample;
      case 'intermediate':
        return mergingExample;
      case 'advanced':
      case 'expert':
        return advancedWorkflowExample;
      default:
        return basicBranchingExample;
    }
  };

  // Helper function to determine if an exercise is a Git basics exercise
  const isGitBasicsExercise = (exercise: Exercise): boolean => {
    // Check if title contains "basics" or "introduction" keywords (case insensitive)
    const lowerTitle = exercise.title.toLowerCase();
    return lowerTitle.includes('basics') || 
            ...
            <Grid item xs={12} md={6} lg={4} key={exercise.id}>
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  ...
                  {/* Add the branch visualization */}
                  {!isGitBasicsExercise(exercise) && (
                    <Box sx={{ my: 2 }}>
                      <Typography variant="caption" fontWeight="bold" color="text.secondary" sx={{ mb: 1, display: 'block' }}>
                        Git Structure:
                      </Typography>
                      <BranchVisualization 
                        initialData={getVisualizationData(exercise)} 
                        interactive={false}
                      />
                    </Box>
                  )}
